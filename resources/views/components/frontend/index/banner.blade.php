...
state('carouselActive', 0);

$banners = computed(fn() => Banner::active()
    ->withCurrentTranslations()
    ->ordered()
    ->get());

$carouselItems = computed(fn() => count($this->banners));

$setCarousel = fn($index) => $this->carouselActive = $index;

?>

@volt('banner-component')
    ...
    <div class="w-full relative">
        ...
        <div class="relative h-screen w-full">
            @foreach($this->banners as $index => $banner)
                ...
                <div x-show="carouselActive === {{ $index }}" x-transition:enter="carousel-fade"
                    ...
                    <div class="fluid-container h-full flex items-center">
                        <div class="w-full text-center text-white fade-in">
                            <h1 class="text-4xl md:text-6xl font-bold mb-6">
                                <span class="block">{{ $banner->getTranslation('title') ?? $banner->title }}</span>
                            </h1>
                            <p class="text-xl md:text-2xl mb-10 w-full max-w-4xl mx-auto">
                                {{ $banner->getTranslation('description') ?? $banner->description }}
                            </p>
                            <a href="{{ $banner->button_link }}"
                               class="px-8 py-3 bg-primary-500 hover:bg-primary-600 text-white rounded-lg font-medium transition-colors shadow-lg">
                                {{ $banner->getTranslation('button_text') ?? $banner->button_text }}
                            </a>
                        </div>
                    </div>
                </div>
