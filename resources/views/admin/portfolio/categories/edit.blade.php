        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm dark:bg-zinc-800 sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form action="{{ route('admin.portfolio.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                @if($category->cover)
                                    <div class="mb-2">
                                        <img src="{{ Storage::url($category->cover) }}" alt="{{ $category->name }}" class="w-32 h-24 object-cover rounded">
                                    </div>
                                @endif
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" id="description" rows="3"
                                    class="mt-1 block w-full rounded-md border-zinc-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white">{{ old('description', $category->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
