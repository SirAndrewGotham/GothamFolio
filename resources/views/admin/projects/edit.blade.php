@push('scripts')
    <script>
        ...
        function confirmDeleteImage(imageId) {
            if (confirm('{{ __('admin.portfolio.projects.confirm_delete_image') }}')) {
                const form = document.getElementById('delete-image-form');
                form.action = '{{ route("admin.project-images.destroy", ":imageId") }}'.replace(':imageId', imageId);
                form.submit();
            }
        }
    </script>
                    ...
                    <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required
                                    ...
                                    <option value="Active" {{ $normalizedStatus == 'active' ? 'selected' : '' }}>{{ __('admin.portfolio.projects.active') }}</option>
                                    <option value="completed" {{ $normalizedStatus == 'completed' ? 'selected' : '' }}>{{ __('admin.portfolio.projects.completed') }}</option>
                                    <option value="in_progress" {{ $normalizedStatus == 'in_progress' ? 'selected' : '' }}>{{ __('admin.portfolio.projects.in_progress') }}</option>
                                    <option value="planned" {{ $normalizedStatus == 'planned' ? 'selected' : '' }}>{{ __('admin.portfolio.projects.planned') }}</option>
                                </select>
                                @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name="duration" id="duration"
                                       class="mt-1 block w-full rounded-md border-zinc-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white"
                                       value="{{ old('duration', $project->duration) }}" placeholder="e.g., 3 months">
                                @error('duration')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                ...
                                <div class="md:col-span-1">
                                    ...
                                    @if($languages && count($languages) > 0)
                                        @foreach($languages as $language)
                                            <div class="mb-4">
                                                <label for="name_{{ $language->code }}" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ strtoupper($language->code) }} Name @if($language->code === 'en')* @endif</label>
                                                <input type="text" name="name[{{ $language->code }}]" id="name_{{ $language->code }}" @if($language->code === 'en')required @endif
                                                class="mt-1 block w-full rounded-md border-zinc-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white"
                                    ...
                                    @else
                                        <div class="mb-4">
                                            <label for="name_en" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">EN Name *</label>
                                ...
                                <div class="md:col-span-1">
                                    ...
                                    @if($languages && count($languages) > 0)
                                        @foreach($languages as $language)
                                            <div class="mb-4">
                                                <label for="description_{{ $language->code }}" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ strtoupper($language->code) }} Description</label>
                                    ...
                                    @else
                                        <div class="mb-4">
                                            <label for="description_en" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">EN Description</label>
                                            <textarea name="description[en]" id="description_en" rows="3"
                                                      class="mt-1 block w-full rounded-md border-zinc-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white">{{ old('description.en', $project->getTranslation('description', 'en')) }}</textarea>
                        ...
                        <div class="mt-8 pt-6 border-t border-zinc-200 dark:border-zinc-700">
                            ...
                            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                ...
                                <div class="md:col-span-2">
                                    <label for="image" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">{{ __('admin.portfolio.projects.project_cover_image') }}</label>
                                ...
                                <div>
                                    ...
                                    <input type="url" name="github_url" id="github_url"
                                           class="mt-1 block w-full rounded-md border-zinc-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-zinc-700 dark:border-zinc-600 dark:text-white"
                                           value="{{ old('github_url', $project->github_url) }}" placeholder="https://">
                                    @error('github_url')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                        ...
                        <div class="mt-8 pt-6 border-t border-zinc-200 dark:border-zinc-700">
                            ...
                            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                ...
                                <!-- Categories -->
                                <div>
                                    <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">{{ __('admin.portfolio.projects.categories') }}</label>
                                    <div class="space-y-2 max-h-48 overflow-y-auto border border-zinc-300 dark:border-zinc-600 rounded-md p-3">
                                        @foreach($categories as $category)
                                            <div class="flex items-center">
                                                ...
                                                <label for="category_{{ $category->id }}" class="ml-2 text-sm text-zinc-700 dark:text-zinc-300">
                                                    {{ $category->name }}
                                                </label>
                                            </div>
                                        @endforeach
                        ...
                        <div class="mt-8 pt-6 border-t border-zinc-200 dark:border-zinc-700">
                            ...
                            <div class="md:col-span-2">
                                <label for="project_images" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">{{ __('admin.portfolio.projects.project_images') }}</label>
                                ...
                                <div class="relative group">
                                    ...
                                    @if($project->images->count() > 0)
                                        <div class="mt-6">
                                            <h4 class="text-md font-medium text-zinc-700 dark:text-zinc-300 mb-4">{{ __('admin.portfolio.projects.existing_images') }}</h4>
                                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                @foreach($project->images as $image)
                                                    <div class="relative group">
                                                        <img src="{{ Storage::url($image->image_path) }}" class="w-full h-32 object-cover rounded-lg shadow-md border border-zinc-200 dark:border-zinc-600" alt="{{ $image->alt_text }}">
                                                        <button type="button" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow-md hover:bg-red-600 transition-colors duration-200" onclick="confirmDeleteImage({{ $image->id }})">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                                            </svg>
                                                        </button>
