...
<x-backend.layouts.app :title="__('admin.gallery.edit_gallery', ['title' => $gallery->title])">
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // Show preview if there's an existing cover image
                @if($gallery->cover_image)
                    previewImage.src = '{{ asset('storage/' . $gallery->cover_image) }}';
                previewContainer.classList.remove('hidden');
                @endif
            });
        </script>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">{{ __('admin.gallery.edit_gallery', ['title' => $gallery->title]) }}</h1>
        ...
        <form action="{{ route('admin.galleries.update', $gallery) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="bg-white rounded-lg shadow p-6 space-y-6">
                        ...
                        @foreach($activeLanguages as $language)
                            <div class="mb-4">
                                ...
                                <input type="text" name="title[{{ $language->code }}]" id="title_{{ $language->code }}"
                                       value="{{ old('title.' . $language->code, $gallery->getTranslation('title', $language->code)) }}" required
                        ...
                        <div class="mb-4">
                            <label class="flex items-center">
                                ...
                                <input type="checkbox" name="is_featured" value="1"
                                       {{ (old('is_featured', $gallery->is_featured) == 1) ? 'checked' : '' }}
                                       class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        ...
                        @foreach($activeLanguages as $language)
                            <div class="mb-4">
                                ...
                                <textarea name="description[{{ $language->code }}]" id="description_{{ $language->code }}" rows="3"
                                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description.' . $language->code, $gallery->getTranslation('description', $language->code)) }}</textarea>
                ...
                <div class="flex justify-end space-x-3">
                    <a href="{{ route('admin.galleries.show', $gallery) }}" class="btn btn-gray">{{ __('admin.gallery.cancel') }}</a>
                    <button type="submit" class="btn btn-primary">{{ __('admin.gallery.update_gallery') }}</button>
                </div>
            </div>
        </form>
    </div>
