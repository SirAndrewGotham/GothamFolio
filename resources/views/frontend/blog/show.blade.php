@push('section-styles')
    <style>
        /* DIAGNOSTIC: Force everything visible */
        /* DIAGNOSTIC: Force visibility for debugging */
        /*.hidden.lg\:block {*/
        ...
        /*!* Override fade-in for debugging *!*/
        /*.fade-in {*/
        /*    opacity: 1 !important;*/
...
<x-frontend.layouts.app section="blog">
                        ...
                        <div class="blog-content fade-in">
                            ...
                            <nav class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400 mb-6">
                                ...
                                <a href="/blog/category/{{ $post->categories->first()->slug ?? 'programming' }}"
                                   class="hover:text-primary-500 transition-colors">{{ $post->categories->first()->name ?? 'Programming' }}</a>
                                <span class="text-gray-400">/</span>
                                <span class="text-primary-500">Article</span>
                            </nav>
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <div
                                        class="text-sm text-gray-600 dark:text-gray-400">{{ $post->author->bio ?? 'Web Developer & Photographer' }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-4 gap-12">
                            ...
                            <div class="hidden lg:block fade-in">
                                    ...
                                    <div class="table-of-contents">
                                        ...
                                        <nav class="space-y-2">
                                            @foreach($post->toc as $item)
                                                <a href="#{{ $item['id'] }}"
                                                   class="block text-sm text-gray-700 dark:text-gray-300 hover:text-primary-500 transition-colors py-1 pl-{{ ($item['level'] - 2) * 2 }}">
                                                    {{ $item['text'] }}
                                                </a>
                                            @endforeach
                                        </nav>
                                ...
                                <div class="mt-6 share-buttons">
                                    <h4 class="font-semibold text-gray-900 dark:text-white mb-3">Share</h4>
                                ...
                                <article class="blog-content fade-in">
                                    ...
                                    <div class="lg:hidden mb-8">
                                        <button @click="showTableOfContents = !showTableOfContents"
                                            ...
                                            <span class="font-semibold">Article Contents</span>
                                        ...
                                        <div x-show="showTableOfContents"
                                            ...
                                            <nav class="space-y-2">
                                                @if($post->toc && count($post->toc) > 0)
                                                    @foreach($post->toc as $item)
                                                        <a href="#{{ $item['id'] }}"
                                                           class="block text-gray-700 dark:text-gray-300 hover:text-primary-500 transition-colors py-1 pl-{{ ($item['level'] - 2) * 2 }}">
                                                            {{ $item['text'] }}
                                                        </a>
                                                    @endforeach
                                                @else
...
{{--                                                    </p>--}}
                                        ...
                                        <div class="blog-content fade-in">
                                            ...
                                            <div class="flex items-center justify-between mb-8">
                                                ...
                                                <div class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                                                    ...
                                                    <span
                                                        id="comments-count">{{ $post->comments->count() }} comments</span>
                                                </div>
                                            </div>
                                                ...
                                                <div class="flex items-center justify-between">
                                                    <div>
                                                        <h3 class="font-semibold text-gray-900 dark:text-white mb-1">
                                                            {{ config('gothamfolio.comments.allow_comments') ? 'Comments enabled' : 'Comments disabled' }}
                                                        </h3>
                                                        <p class="text-sm text-gray-600 dark:text-gray-400">
                                                            {{ config('gothamfolio.comments.allow_comments') ? 'Administrator has allowed discussion for this article' : 'Comments are currently disabled for this article' }}
                                                        </p>
                                                    </div>
                                                    <div
                                                        ...
                                                        {{ config('gothamfolio.comments.allow_comments') ? 'Enabled' : 'Disabled' }}
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            async submitComment() {
                                ...
                                const newComment = {
                                    ...
                                    date: new Date().toLocaleDateString('{{ $current_locale ?? 'en-US' }}'),
                                    avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(this.name)}&background=0ea5e9&color=fff`,
                                    replies: []
                                };
                                // Reset form
                                this.name = '';
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <div
                                                                            ...
                                                                            @if($comment->author && $comment->author->id === $post->author_id)
                                                                                <span>•</span>
                                                                                <span
                                                                                    class="text-primary-500">Author</span>
                                                                            @endif
                                                                        </div>
                                                                    </div>
                                                                    <button
                                                ...
                                                @if($post->comments->count() === 0)
                                                    <div class="text-center py-12">
                                                        ...
                                                        <h3 class="text-xl font-semibold text-gray-600 dark:text-gray-400 mb-2">
                                                            No comments yet</h3>
                                                        <p class="text-gray-500 dark:text-gray-500">Be the first to
